...
const Profile = () => {
    ...
    const get_loading=false // fix it here
    const [inputval, setInputval] = useState(loggedInUser?.username)
    const dispatch: Dispatch<any> = useDispatch()
    useEffect(() => {
        if(loggedInUser){
            dispatch(getOneUserPost(loggedInUser._id))
        }
    ...
    const usernameChange = () => {
        if(loggedInUser._id){
            dispatch(updateUserdata({username:inputval},loggedInUser._id))
        }
        setInputval("")
        setEditUser(false)
    }
    ...
    return <>
        ...
        <div className='px-1 md:m-auto py-14' >
            <div className='flex h-40 items-center justify-between md:w-[50%] m-auto relative'>
                <Image src={`${loggedInUser.cover||"/logod.png"}`} width={1200} height={1200} alt='Cover Photo' className='border-[1px] border-blue-400 rounded-xl absolute z-0 w-full h-full' blurDataURL={BlurImage}/>
                <div className='border-2 ml-4 rounded-full relative border-blue-400/60'>
                    <Image className="rounded-full w-24 h-24" src={`${loggedInUser.image}`} width={100} height={100} alt="loggedInUser.name" blurDataURL={BlurImage} />
                    <div className={`w-4 h-4 absolute bottom-2 right-0 rounded-full ${loggedInUser? "bg-green-500" : "bg-red-500"}`} ></div>
                </div>
                <div className='flex flex-col items-center mr-4 bg-black/50 z-10 py-2 relative'>
                    <p className='text-xl border-b-2 border-blue-300/40 px-4 mb-2 pb-2 z-10'>{loggedInUser.name}</p>
                    {editUser ? <div className='flex items-center justify-end w-max-lg'>
                        ...
                        <p className='text-gray-200 hover:underline font-semibold'>{loggedInUser.username}</p>
